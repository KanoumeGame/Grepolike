...
const resolveBattle = (attackingUnits, defendingUnits, unitType, attackerPhalanx, attackerSupport, defenderPhalanx, defenderSupport, attackingHero, defendingHero) => {
    //  Check if either side has units of the required type or a hero
    ...
    //  If the defender has no relevant units OR hero, the attacker automatically wins this phase.
    ...
    //  If the attacker has no relevant combat units OR hero, but the defender does, the attacker loses.
    ...
    const calculateEffectivePower = (units, opponentUnits, isAttacker, phalanxUnit, supportUnit, heroId) => {
        ...
        for (const unitId in units) {
            ...
            //  Apply hero passives correctly to attack or defense
    ...
    //  Simplified loss calculation for more robust and predictable outcomes.
...
export function resolveCombat(attackingUnits, defendingUnits, defendingResources, isNavalAttack, attackerPhalanx, attackerSupport, defenderPhalanx, defenderSupport, attackingHero, defendingHero) {
    ...
    //  Calculate battle points for both sides
...
export function resolveVillageRetaliation(playerUnits) {
    ...
    //  village retaliation causes a flat 5% loss of total army population value
    let populationToLose = totalPopulation * 0.05;

    //  distribute losses proportionally among land units
...
export function resolveScouting(targetGameState, attackingSilver) {
    const defenderCaveSilver = targetGameState.cave?.silver || 0;
    const attackerEspionagePower = attackingSilver;

    // The success of the scout is determined by comparing the attacker's silver with the defender's.
    // A random factor is included to make it less predictable.
    const success = (attackerEspionagePower * (0.9 + Math.random() * 0.2)) > (defenderCaveSilver * (0.9 + Math.random() * 0.2));

    if (success) {
        // If scouting is successful, return a report of the target's resources and units.
        return {
            success: true,
            ...
            god: targetGameState.god || 'None',
            silverLostByDefender: 0
        };
    } else {
        // If scouting fails, the defender's cave is depleted as they use silver to counter the spy.
        const silverLostByDefender = Math.min(defenderCaveSilver, attackingSilver);
        return {
            success: false,
            message: 'Scouting failed! Your spy was detected and eliminated.',
            silverGained: 0, // Defender no longer gains silver
            silverLostByDefender: silverLostByDefender
        };
    }
