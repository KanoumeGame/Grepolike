...
export const sendSystemMessage = async (targetUserId, targetUsername, messageText, worldId) => {
    ...
    //  Use a sorted array for a consistent document ID/query
    ...
    if (conversationSnapshot.empty) {
        ...
        await setDoc(conversationRef, {
            ...
            readBy: [], //  Initialize as unread for the target
        });
    } else {
    ...
    //  Update the conversation's last message and mark as unread for the recipient
    await updateDoc(conversationRef, {
        lastMessage: { text: messageText.substring(0, 30) + '...', senderId: systemId, timestamp: serverTimestamp() },
        readBy: [], //  Reset readBy to ensure it appears as a new message
    });
};
