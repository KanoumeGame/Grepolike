...
//  Gets the specific unit ID for a reward based on a player's nation
...
//  Helper to find the generic type of a national unit
...
export const getTrainableUnits = (nation) => {
    const nationalUnits = Object.keys(unitConfig).filter(id => {
        ...
        //  Ensure we only get LAND units that are nation-specific
    ...
    //  Return only the national units, excluding the generic ones.
...
export const getTrainableNavalUnits = (nation) => {
    const nationalUnits = Object.keys(unitConfig).filter(id => {
        const unit = unitConfig[id];
        //  Make sure we only get nation-specific NAVAL units
    ...
    //  Add generic NAVAL units that are not nation-specific
