...
const AdminCheatMenu = ({ onCheat, onClose, InstantBuildActive}) => {
    const [amounts, setAmounts] = useState({ wood: 0, stone: 0, silver: 0, platinum: 0, runecoins: 0 });
    ...
    const [favorAmount, setFavorAmount] = useState(0); //  Re-added favor cheat state
    ...
    //  Handler for the new "Found Second City" button
    const handleFoundCity = () => {
        onCheat({}, {}, 0, false, '', false, false, 0, true, false, false); 
        onClose();
    };

    //  Handler for the "Force Refresh Data" button
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70" onClick={onClose}>
            <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md border-2 border-gray-600 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                <h3 className="font-title text-2xl text-white mb-4 flex-shrink-0">Admin Cheats</h3>
