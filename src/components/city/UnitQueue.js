...
import './UnitQueue.css';
import PlatinumIcon from '../icons/PlatinumIcon'; // Import PlatinumIcon
...
const UnitQueueItem = ({ item, onCancel, isFirst, isLast, onHover, onLeave }) => {
    ...
    useEffect(() => {
        ...
        let interval; //  define interval inside useEffect
        ...
        calculateTimeLeft();
        interval = setInterval(calculateTimeLeft, 1000); //  assign interval here
...
const UnitQueue = ({ unitQueue, onCancel, title = "In Training", onSpeedUp }) => {
    const [hoveredItem, setHoveredItem] = useState(null);
    const [tooltipStyle, setTooltipStyle] = useState({ display: 'none' });
    const tooltipTimeoutRef = useRef(null);
    const queueContainerRef = useRef(null);

    //  handle mouse enter to show tooltip and calculate its position
    ...
    //  Get the completion time of the last item in the queue
    ...
    const renderTooltip = () => {
        ...
        const isFirst = unitQueue && unitQueue.length > 0 && unitQueue[0].id === hoveredItem.id;

        return (
            <div 
                className="unit-queue-tooltip" 
                style={tooltipStyle}
            ...
            >
                ...
                {isFirst && onSpeedUp && (
                    <button onClick={() => onSpeedUp(hoveredItem)} className="btn btn-primary mt-2 w-full">
