...
const Chat = ({ isVisible, onClose }) => {
    ...
    //  fetch messages based on the active tab (world or alliance)
    useEffect(() => {
        if (!worldId) return;
    ...
    //  send a new message to the currently active chat tab
    ...
    //  handle mouse down for dragging the chat window
