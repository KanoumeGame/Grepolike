...
import ManagementPanel from './management/managementPanelpls'
...
const Game = ({ onBackToWorlds }) => {
    ...
    return (
        <div className="w-full h-screen bg-gray-900 text-white relative">
            ...
            <div className="pouch-toggle-button-container">
                <button onClick={() => setIsPouchOpen(prev => !prev)} className="pouch-toggle-button">
                    🎒
                </button>
            </div>
            {isPouchOpen && <Pouch items={playerGameData?.items || {}} onClose={() => setIsPouchOpen(false)} />}
