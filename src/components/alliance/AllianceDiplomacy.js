...
//  Cache for diplomacy data to reduce reads.
...
//  Local cache for alliance cities to reduce N+1 queries.
...
const AllianceDiplomacy = () => {
    ...
    //  Fetch all alliances, world cities, and alliance cities for autocomplete and offers
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            //  Use local cache for alliance cities to prevent N+1 queries on every view
            if (now - allianceCitiesCache.timestamp < CACHE_DURATION && allianceCitiesCache.cities) {
                ...
                //  Fetch cities from all members in parallel for better performance
