        ...
        if (movement.status === 'returning') {
            ...
            switch (movement.type) {
                case 'attack_god_town': {
                    ...
                    batch.set(playerProgressRef, { 
                        damageDealt: currentDamage + damageDealt,
                    ...
                    if (newHealth === 0) {
                        ...
                        const participantsRef = collection(db, 'worlds', worldId, 'godTowns', movement.targetTownId, 'playerProgress');
                        const participantsSnap = await getDocs(participantsRef);
                        const damageThreshold = 5000;

                        for (const participantDoc of participantsSnap.docs) {
                            const participantId = participantDoc.id;
                            const participantData = participantDoc.data();
                            if (participantData.damageDealt >= damageThreshold) {
                                const gameDocRef = doc(db, `users/${participantId}/games`, worldId);
                                const gameDoc = await getDoc(gameDocRef);
                                if (gameDoc.exists()) {
                                    const gameData = gameDoc.data();
                                    const newItems = { ...(gameData.items || {}) };
                                    const rewardItem = Math.random() < 0.5 ? "attack_boost_5" : "defense_boost_5";
                                    newItems[rewardItem] = (newItems[rewardItem] || 0) + 1;
                                    batch.update(gameDocRef, { items: newItems });
                                    const rewardReport = {
                                        type: 'misc', title: `God Town Defeated!`, timestamp: serverTimestamp(),
                                        outcome: { message: `For your valiant participation in the defeat of ${townData.name}, you have been awarded a ${itemsConfig[rewardItem].name}!` },
                                        read: false,
                                    };
                                    batch.set(doc(collection(db, `users/${participantId}/worlds/${worldId}/reports`)), rewardReport);
                                }
                            }
                        }
                        const finalBlowGameRef = doc(db, `users/${movement.originOwnerId}/games`, worldId);
                        const finalBlowGameDoc = await getDoc(finalBlowGameRef);
                        if (finalBlowGameDoc.exists()) {
                            const gameData = finalBlowGameDoc.data();
                            const newItems = { ...(gameData.items || {}) };
                            const rareItems = ["training_speed_10", "movement_speed_10"];
                            const rewardItem = rareItems[Math.floor(Math.random() * rareItems.length)];
                            newItems[rewardItem] = (newItems[rewardItem] || 0) + 1;
                            batch.update(finalBlowGameRef, { items: newItems });
                            const finalBlowReport = {
                                type: 'misc', title: `Final Blow on ${townData.name}!`, timestamp: serverTimestamp(),
                                outcome: { message: `You dealt the final blow to ${townData.name} and received a rare ${itemsConfig[rewardItem].name}!` },
                                read: false,
                            };
                            batch.set(doc(collection(db, `users/${movement.originOwnerId}/worlds/${worldId}/reports`)), finalBlowReport);
                        }
                    } else {
                ...
                case 'attack_village': {
                    ...
                    if (result.attackerWon) {
                        ...
                        //  Add quest event on successful village attack
                ...
                case 'attack': {
                    ...
                    if (result.attackerWon) {
                        const questEventRef = doc(collection(db, `users/${movement.originOwnerId}/games/${worldId}/questEvents`));
                        batch.set(questEventRef, { type: 'attack_player', timestamp: serverTimestamp() });
                    }
                ...
                case 'scout': {
                    ...
                    if (result.success) {
                        ...
                        // Update the defender's cave based on the silver they lost countering the spy.
                        ...
                        const spyCaughtReport = {
                            ...
                            silverGained: 0, // No silver is gained
                            read: false,
                        };
