...
export const useAdminActions = ({
    ...
    const handleCheat = async (amounts, troop, farmLevels, warehouseLevels, isInstantBuild, unresearchId, isInstantResearch, isInstantUnits, favorAmount, foundSecondCity, forceRefresh, healHero) => {
        ...
        if (amounts.platinum && amounts.platinum > 0) {
            ...
            try {
                await runTransaction(db, async (transaction) => {
                    ...
                    transaction.update(gameDocRef, { platinum: currentPlatinum + amounts.platinum });
                });
            } catch (error) {
                setMessage(`Failed to add platinum: ${error.message}`);
                console.error(error);
            }
        }
        ...
        const otherCheatsApplied = amounts.wood > 0 || amounts.stone > 0 || amounts.silver > 0 || troop.amount > 0 || farmLevels > 0 || warehouseLevels > 0 || unresearchId || favorAmount > 0;
        if (otherCheatsApplied) {
