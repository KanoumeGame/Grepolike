    ...
    if (gameState.buildings) {
        for (const buildingId in gameState.buildings) {
            ...
            if (buildingInfo && buildingInfo.points) {
                ...
                //  Sum of an arithmetic series for level points
                points += buildingInfo.points * (level * (level + 1) / 2);
            }
        }
    }
    //  Calculate points from units based on population cost
    if (gameState.units) {
...
export const useCityState = (worldId, isInstantBuild, isInstantResearch, isInstantUnits) => {
    ...
    //  Memoized function to get happiness details
    ...
    //  Memoized function to calculate happiness
    ...
    //  Memoized function to get max worker slots
    const getMaxWorkerSlots = useCallback((level) => {
        if (!level || level < 1) return 0;
        return Math.min(6, 1 + Math.floor(level / 5));
    }, []);

    //  Memoized function to get production rates
    const getProductionRates = useCallback((buildings) => {
        ...
        // Apply item boosts
        if (playerGameData?.activeBoosts) {
            const now = new Date();
            for (const boostKey in playerGameData.activeBoosts) {
                const boost = playerGameData.activeBoosts[boostKey];
                if (boost.expires.toDate() > now) {
                    if (boostKey.startsWith('production_boost_')) {
                        const resource = boostKey.replace('production_boost_', '');
                        if (rates[resource]) {
                            rates[resource] *= (1 + boost.value);
                        }
                    }
                }
            }
        ...
        return rates;
    }, [calculateHappiness, cityGameState, activeCityId, playerAlliance, playerGameData]);

    //  Memoized function to get warehouse capacity
    ...
    //  Memoized function to get farm capacity
    ...
    //  Memoized function to get hospital capacity
    const getHospitalCapacity = useCallback((level) => {
        if (!level) return 0;
        return level * 1000;
    }, []);

    //  Memoized function to get upgrade cost
    const getUpgradeCost = useCallback((buildingId, level) => {
        ...
        let calculatedTime = Math.floor(cost.time * Math.pow(1.25, level - 1));

        // Apply construction speed boost
        if (playerGameData?.activeBoosts?.construction_speed) {
            const boost = playerGameData.activeBoosts.construction_speed;
            if (boost.expires.toDate() > new Date()) {
                calculatedTime *= (1 - boost.value);
            }
        }
    ...
    }, [isInstantBuild, playerGameData]);

    //  Memoized function to get research cost
    const getResearchCost = useCallback((researchId) => {
        ...
        // Apply research speed boost
        if (playerGameData?.activeBoosts?.research_speed) {
            const boost = playerGameData.activeBoosts.research_speed;
            if (boost.expires.toDate() > new Date()) {
                time *= (1 - boost.value);
            }
        }
    ...
    }, [isInstantResearch, playerAlliance, playerGameData]);

    //  Memoized function to calculate used population
    ...
    //  Memoized function to calculate total points
    ...
    //  Effect to subscribe to city data changes
    useEffect(() => {
